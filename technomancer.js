...
var mainState = {
    ...
    preload: function () {
        game.load.image('background', 'assets/city.jpg');
        game.load.image('mainCharac', 'assets/mainchar3.gif');
        game.load.image('robot', 'assets/robots2.png');
    ...
    create: function () {
		...
		this.enemy = this.spritecreator('robot',0.2,200);
        ...
        //this.sword = this.spritecreator ('weapon', 0.2,100); 
    ...
    update: function () {
        ...
        if (time > 30) {
            for (var i = 0; i < this.enemies.length; i++) {
                 this.robot = this.enemies.getChildAt(i);
                 this.robot.body.velocity.x = this.robot.body.velocity.x * (Math.round(Math.random()) * 2 - 1);
            }
            time = 0;
        } else {
            time++;
        ...
        for (var i = 0; i < this.enemies.length; i++) {
            ...
            if (this.robot.body.velocity.x === 0){
                this.robot.body.velocity.x =  (150 + score1 * 2) * (Math.round(Math.random()) * 2 - 1);
            }
        }
        ...
        if (time2 > 60 - tempScore * 0.6) {
            this.temp = this.spritecreator('robot',0.2, Math.random() * 1000 - 0);
